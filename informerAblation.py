import pandas as pd
import numpy as np
import torch
import torch.nn as nn
from torch.utils.data import Dataset, DataLoader
from sklearn.preprocessing import MinMaxScaler
import os
import re

# --- 1. CONFIGURATION ---
TRAIN_PATH = "/home/tom_mscds24/data/toyota dataset/train_data.csv"
TEST_PATH = "/home/tom_mscds24/data/toyota dataset/test_data.csv"
OUTPUT_FILE = "/home/tom_mscds24/data/toyota dataset/Informer_Ablation_Full_Results.xlsx"

# The 3 Windows from the Table
WINDOWS = [1, 8, 16] 
INPUT_LEN = 16 # Fixed Look-back

# ABLATION SCENARIOS (Name -> {Flags})
ABLATION_SCENARIOS = {
    "Informer":                         {"distil": True,  "dec_only": False, "gen_dec": True,  "attn": "prob"},
    "Informer (Decoder Only)":          {"distil": False, "dec_only": True,  "gen_dec": True,  "attn": "prob"},
    "with Full Attention":              {"distil": True,  "dec_only": False, "gen_dec": True,  "attn": "full"},
    "w/o Self-attn Distilling":         {"distil": False, "dec_only": False, "gen_dec": True,  "attn": "prob"},
    "w/o ProbSparse Attn & Distilling": {"distil": False, "dec_only": False, "gen_dec": True,  "attn": "full"},
    "w/o Generative Decoder":           {"distil": True,  "dec_only": False, "gen_dec": False, "attn": "prob"},
}

# Hyperparameters
BATCH_SIZE = 32
EPOCHS = 5
LR = 0.0001
DEVICE = torch.device("cuda" if torch.cuda.is_available() else "cpu")

FEATURE_COLS = [
    'Close', 'SMA', 'RSI', 'OBV', 'ADX',
    'Gold_Close', 'USD_JPY_Close', 'sentiment_score'
]

# --- 2. INFORMER MODEL COMPONENTS ---
class DistillingLayer(nn.Module):
    def __init__(self, c_in):
        super(DistillingLayer, self).__init__()
        self.conv = nn.Conv1d(c_in, c_in, kernel_size=3, padding=1)
        self.norm = nn.BatchNorm1d(c_in)
        self.activation = nn.ELU()
        self.max_pool = nn.MaxPool1d(kernel_size=3, stride=2, padding=1)

    def forward(self, x):
        x = x.permute(0, 2, 1)
        x = self.max_pool(self.activation(self.norm(self.conv(x))))
        x = x.permute(0, 2, 1)
        return x

class InformerEncoderLayer(nn.Module):
    def __init__(self, d_model, n_heads, d_ff=512, dropout=0.05):
        super(InformerEncoderLayer, self).__init__()
        self.attn = nn.MultiheadAttention(d_model, n_heads, batch_first=True)
        self.conv1 = nn.Conv1d(d_model, d_ff, kernel_size=1)
        self.conv2 = nn.Conv1d(d_ff, d_model, kernel_size=1)
        self.norm1 = nn.LayerNorm(d_model)
        self.norm2 = nn.LayerNorm(d_model)
        self.dropout = nn.Dropout(dropout)
        self.activation = nn.ReLU()

    def forward(self, x):
        new_x, _ = self.attn(x, x, x)
        x = x + self.dropout(new_x)
        y = x = self.norm1(x)
        y = self.dropout(self.activation(self.conv1(y.transpose(-1, 1))))
        y = self.dropout(self.conv2(y).transpose(-1, 1))
        return self.norm2(x + y)

class InformerEncoder(nn.Module):
    def __init__(self, enc_in, d_model, n_heads, e_layers=2, distil=True):
        super(InformerEncoder, self).__init__()
        self.enc_embedding = nn.Linear(enc_in, d_model)
        self.layers = nn.ModuleList()
        self.distill_layers = nn.ModuleList()
        self.distil = distil
        
        for i in range(e_layers):
            self.layers.append(InformerEncoderLayer(d_model, n_heads))
            if self.distil and i < e_layers - 1:
                self.distill_layers.append(DistillingLayer(d_model))
        self.norm = nn.LayerNorm(d_model)

    def forward(self, x):
        x = self.enc_embedding(x)
        for i, layer in enumerate(self.layers):
            x = layer(x)
            if self.distil and i < len(self.distill_layers):
                x = self.distill_layers[i](x)
        return self.norm(x)

class InformerDecoderLayer(nn.Module):
    def __init__(self, d_model, n_heads, d_ff=512, dropout=0.05):
        super(InformerDecoderLayer, self).__init__()
        self.self_attn = nn.MultiheadAttention(d_model, n_heads, batch_first=True)
        self.cross_attn = nn.MultiheadAttention(d_model, n_heads, batch_first=True)
        self.conv1 = nn.Conv1d(d_model, d_ff, kernel_size=1)
        self.conv2 = nn.Conv1d(d_ff, d_model, kernel_size=1)
        self.norm1 = nn.LayerNorm(d_model)
        self.norm2 = nn.LayerNorm(d_model)
        self.norm3 = nn.LayerNorm(d_model)
        self.dropout = nn.Dropout(dropout)
        self.activation = nn.ReLU()

    def forward(self, x, cross, tgt_mask=None, cross_mask=None):
        x = x + self.dropout(self.self_attn(x, x, x, attn_mask=tgt_mask)[0])
        x = self.norm1(x)
        if cross is not None:
            x = x + self.dropout(self.cross_attn(x, cross, cross)[0])
        x = self.norm2(x)
        y = x
        y = self.dropout(self.activation(self.conv1(y.transpose(-1, 1))))
        y = self.dropout(self.conv2(y).transpose(-1, 1))
        return self.norm3(x + y)

class InformerDecoder(nn.Module):
    def __init__(self, dec_in, d_model, n_heads, d_layers=1):
        super(InformerDecoder, self).__init__()
        self.dec_embedding = nn.Linear(dec_in, d_model)
        self.layers = nn.ModuleList([InformerDecoderLayer(d_model, n_heads) for _ in range(d_layers)])
        self.norm = nn.LayerNorm(d_model)
        self.projection = nn.Linear(d_model, 1)

    def forward(self, x, cross, tgt_mask=None):
        x = self.dec_embedding(x)
        for layer in self.layers:
            x = layer(x, cross, tgt_mask=tgt_mask)
        x = self.norm(x)
        x = self.projection(x)
        return x

class Informer(nn.Module):
    def __init__(self, enc_in, dec_in, out_len, d_model=64, n_heads=4, e_layers=2, d_layers=1, 
                 distil=True, dec_only=False):
        super(Informer, self).__init__()
        self.out_len = out_len
        self.dec_only = dec_only
        
        if not self.dec_only:
            self.encoder = InformerEncoder(enc_in, d_model, n_heads, e_layers, distil=distil)
        
        self.decoder = InformerDecoder(dec_in, d_model, n_heads, d_layers)

    def forward(self, x_enc, x_dec):
        if self.dec_only:
            enc_out = None
        else:
            enc_out = self.encoder(x_enc)
            
        dec_seq_len = x_dec.shape[1]
        tgt_mask = torch.triu(torch.ones(dec_seq_len, dec_seq_len) * float('-inf'), diagonal=1).to(x_dec.device)
        
        dec_out = self.decoder(x_dec, enc_out, tgt_mask=tgt_mask)
        return dec_out[:, -self.out_len:, :]

# --- 3. DATA LOADER ---
class InformerDataset(Dataset):
    def __init__(self, data_scaled, seq_len, label_len, pred_len):
        self.data = data_scaled
        self.seq_len = seq_len
        self.label_len = label_len
        self.pred_len = pred_len

    def __len__(self):
        return len(self.data) - self.seq_len - self.pred_len + 1

    def __getitem__(self, index):
        s_begin = index
        s_end = s_begin + self.seq_len
        
        r_begin = s_end - self.label_len
        r_end = r_begin + self.label_len + self.pred_len
       
        enc_input = self.data[s_begin:s_end]
        dec_input = self.data[r_begin:r_end].copy()
        dec_input[-self.pred_len:, :] = 0
        target = self.data[s_end : s_end + self.pred_len, 0:1]
       
        return (torch.tensor(enc_input, dtype=torch.float32),
                torch.tensor(dec_input, dtype=torch.float32),
                torch.tensor(target, dtype=torch.float32))

# --- 4. EXPERIMENT RUNNER ---
def run_ablation_study():
    print("?? Loading Datasets...")
    if not os.path.exists(TRAIN_PATH) or not os.path.exists(TEST_PATH):
        print("? Error: Train/Test CSVs not found.")
        return

    # Load & Scale
    df_train = pd.read_csv(TRAIN_PATH)
    df_test = pd.read_csv(TEST_PATH)
    
    scaler = MinMaxScaler()
    train_vals = df_train[FEATURE_COLS].values
    test_vals = df_test[FEATURE_COLS].values
    scaler.fit(train_vals)
    train_scaled = scaler.transform(train_vals)
    test_scaled = scaler.transform(test_vals)
    
    writer = pd.ExcelWriter(OUTPUT_FILE, engine='xlsxwriter')
    summary_results = {name: {} for name in ABLATION_SCENARIOS.keys()}

    # --- MAIN LOOPS ---
    for scenario_name, config in ABLATION_SCENARIOS.items():
        print(f"\n==========================================")
        print(f"?? ABLATION: {scenario_name}")
        print(f"==========================================")
        
        for out_len in WINDOWS:
            if config["gen_dec"]:
                label_len = INPUT_LEN // 2 
            else:
                label_len = 0

            # --- FIX: SANITIZE SHEET NAME ---
            # Remove invalid chars for Excel: [ ] : * ? / \
            safe_name = re.sub(r'[\[\]:*?/\\]', '', scenario_name) # Remove bad chars
            safe_name = safe_name.replace(' ', '_') # Replace space with underscore
            # Excel limits sheet names to 31 chars
            sheet_name = f"{safe_name[:20]}_{out_len}"
            
            # Dataset
            train_set = InformerDataset(train_scaled, INPUT_LEN, label_len, out_len)
            test_set = InformerDataset(test_scaled, INPUT_LEN, label_len, out_len)
            train_loader = DataLoader(train_set, batch_size=BATCH_SIZE, shuffle=True)
            test_loader = DataLoader(test_set, batch_size=BATCH_SIZE, shuffle=False)

            # Model
            model = Informer(
                enc_in=len(FEATURE_COLS),
                dec_in=len(FEATURE_COLS),
                out_len=out_len,
                d_model=64, n_heads=4,
                distil=config["distil"],
                dec_only=config["dec_only"]
            ).to(DEVICE)
            
            optimizer = torch.optim.Adam(model.parameters(), lr=LR)
            criterion = nn.MSELoss()

            # Train
            model.train()
            for epoch in range(EPOCHS):
                for enc_in, dec_in, target in train_loader:
                    enc_in, dec_in, target = enc_in.to(DEVICE), dec_in.to(DEVICE), target.to(DEVICE)
                    optimizer.zero_grad()
                    output = model(enc_in, dec_in)
                    loss = criterion(output, target)
                    loss.backward()
                    optimizer.step()

            # Test
            model.eval()
            scaled_diffs_sq = []
            
            with torch.no_grad():
                for enc_in, dec_in, target in test_loader:
                    enc_in, dec_in, target = enc_in.to(DEVICE), dec_in.to(DEVICE), target.to(DEVICE)
                    output = model(enc_in, dec_in)
                    diff = (output - target).cpu().numpy()
                    scaled_diffs_sq.append(diff**2)

            if len(scaled_diffs_sq) > 0:
                all_scaled_sq = np.concatenate(scaled_diffs_sq, axis=0)
                mse_scaled = np.mean(all_scaled_sq)
            else:
                mse_scaled = 0.0

            summary_results[scenario_name][out_len] = mse_scaled
            print(f"   -> Out {out_len}: MSE {mse_scaled:.5f}")

            # Save minimal sheet
            pd.DataFrame({'MSE': [mse_scaled]}).to_excel(writer, sheet_name=sheet_name)
            torch.cuda.empty_cache()

    writer.close()
    
    # --- PRINT FINAL TABLE ---
    print("\n\n" + "="*65)
    print(f"{'INFORMER ABLATION STUDY: MEAN SQUARED ERROR (%)':^65}")
    print(f"{'LOOK-BACK: 16 DAYS':^65}")
    print("="*65)
    print(f"{'Model Variant':<35} | {'1 day':<8} | {'8 days':<8} | {'16 days':<8}")
    print("-" * 65)
    
    order = [
        "Informer", "Informer (Decoder Only)", "with Full Attention",
        "w/o Self-attn Distilling", "w/o ProbSparse Attn & Distilling", 
        "w/o Generative Decoder"
    ]
    
    for name in order:
        scores = summary_results.get(name, {})
        s1 = f"{scores.get(1, 0.0):.3f}"
        s8 = f"{scores.get(8, 0.0):.3f}"
        s16 = f"{scores.get(16, 0.0):.3f}"
        print(f"{name:<35} | {s1:<8} | {s8:<8} | {s16:<8}")
    
    print("="*65 + "\n")

if __name__ == "__main__":
    run_ablation_study()